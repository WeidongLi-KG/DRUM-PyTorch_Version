# data.py (Python3 port of original)
import numpy as np
import os
...
def resplit(train, facts, no_link_percent):
    ...
    if no_link_percent == 0.:
        ...
        new_train = all_[:num_train]
        new_facts = all_[num_train:]
    else:
        link_cntr = Counter()
        for tri in all_:
...
class Data(object):
    def __init__(self, folder, seed, type_check, domain_size, no_extra_facts):
        ...
        train_qs = set([q for (q, _, _) in self.train])
        test_qs = set([q for (q, _, _) in self.test])
        aug_train = set([q for (q, _, _) in self._augment_with_reverse(self.train)])
        aug_test = set([q for (q, _, _) in self._augment_with_reverse(self.test)])
        self.query_for_rules = list(train_qs | test_qs | aug_train | aug_test)
        self.parser = self._create_parser()

    def _create_parser(self):
        assert(self.num_query == 2 * len(self.relation_to_number) == 2 * self.num_relation)
        parser = {"query": {}, "operator": {}}
        number_to_relation = {value: key for key, value in self.relation_to_number.items()}
    ...
    def _parse_domains_file(self, file_name):
        ...
        with open(file_name, "r") as f:
            for line in f:
                ...
                if len(this_domain) == self.domain_size:
                    ...
                    remains = [i for i in range(self.num_relation) if i not in this_domain]
        ...
        for r in range(self.num_relation):
            if r not in result.keys():
                result[r] = list(np.random.choice(range(self.num_relation), self.domain_size, replace=False))
        return result
    ...
    def reset(self, batch_size):
        ...
        if not self.type_check:
            ...
            self.num_batch_valid = int(self.num_valid // batch_size // 1 + 1)
    ...
    def _next_batch(self, start, size, samples):
        assert(start < max(1, size))
        end = min(start + self.batch_size, size)
        if self.type_check:
            this_batch_tmp = samples[start:end]
            major_relation = this_batch_tmp[0][0]
            batch_size = next((i for i in range(len(this_batch_tmp)) if this_batch_tmp[i][0] != major_relation), len(this_batch_tmp))
        ...
        this_batch_id = list(range(start, end))
    ...
    def next_train(self):
        self.train_start, this_batch, this_batch_id = self._next_batch(self.train_start, self.num_train, self.train)
        if self.share_db and self.use_extra_facts:
...
class DataPlus(Data):
    def __init__(self, folder, seed):
        ...
        self.kb_relation_to_number = self._numerical_encode_file(self.kb_relation_file)
        self.kb_entity_to_number = self._numerical_encode_file(self.kb_entity_file)
        self.query_vocab_to_number = self._numerical_encode_file(self.query_vocab_file)
        self.test_file = os.path.join(folder, "test.txt")
        self.train_file = os.path.join(folder, "train.txt")
        self.valid_file = os.path.join(folder, "valid.txt")
        self.facts_file = os.path.join(folder, "facts.txt")
    ...
    def _numerical_encode_file(self, file_name):
    ...
    def _create_parser(self):
        parser = {"operator": {}}
        number_to_relation = {value: key for key, value in self.kb_relation_to_number.items()}
        number_to_query_vocab = {value: key for key, value in self.query_vocab_to_number.items()}
    ...
    def is_true(self, q, h, t):
        return tuple(q + [h, t]) in self.all_exams
