...
class Learner(nn.Module):
    ...
    def forward(self, queries, heads, tails, mdb_torch, training=True):
        """
        mdb_torch: list of length num_operator/2, each item is either:
           - tuple (forward, transpose) where forward/transpose are torch tensors (sparse or dense)
           - or a single torch sparse/dense (then transpose computed if needed)
           - or scipy.sparse matrix (legacy, will be converted on the fly to CPU sparse)
        """
        batch = tails.size(0)
        ...
        rnn_seq = rnn_inputs[:, :seq_len, :]  # (batch, seq_len, emb)
        ...
        for i_rank in range(self.rank):
            for t in range(self.num_step):
                memory_read = memories_list[i_rank][:, -1, :]  # (batch, num_entity)
                if t < self.num_step - 1:
                    database_results = []
                    mem_t = memory_read.t()  # (num_entity, batch)

                    # mdb_torch expected length = num_operator/2
                    for r in range(self.num_operator // 2):
                        entry = mdb_torch[r]
                        # entry can be (fw, tr) or single tensor; handle both
                        ...
                        # forward product: op_fw * mem_t -> (num_entity, batch)
                        if is_torch_sparse(op_fw):
                            prod = torch.sparse.mm(op_fw, mem_t)  # (num_entity, batch)
                            prod_tb = prod.t()
                        elif is_torch_dense(op_fw):
                            # dense forward
                            prod = torch.matmul(op_fw, mem_t)  # (num_entity, batch)
                            prod_tb = prod.t()
                        else:
                            # fallback: scipy.sparse -> convert to CPU torch sparse
                        ...
                        if op_tr is not None:
                            if is_torch_sparse(op_tr):
                                ...
                                # build transpose from op_fw (if op_tr not provided)
                                # convert op_fw dense to transpose
                                if is_torch_dense(op_fw):
                                    prod2 = torch.matmul(op_fw.t(), mem_t)
                                    prod2_tb = prod2.t()
                                else:
                                    # last resort: convert to dense (expensive)
                    ...
                    # identity operator (memory_read) with last attention column
